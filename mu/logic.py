...
import platformdirs
from mu.interface.qt import QMessageBox, QObject, pyqtSignal, QtCore
...
class Editor(QObject):
    ...
    def __init__(self, view):
        ...
        self.theme = "night"
        self.mode = "csharp"
        self.python_extensions = [".cs", ".csproj", ".sln", ".json", ".xaml"]
        self.modes = {}
    ...
    def setup(self, modes):
        ...
        default_mode = "csharp" if "csharp" in self.modes else list(self.modes.keys())[0]
        wd = self.modes[default_mode].workspace_dir()
    ...
    def restore_session(self, paths=None):
        """
        Attempts to recreate the tab state from the last time the editor was run.
        """
        old_session = settings.session
        logger.debug(old_session)
        if "theme" in old_session:
            self.theme = old_session["theme"]
        if self.theme not in ("night", "contrast"):
            self.theme = "night"
        self._view.set_theme(self.theme)
        if "mode" in old_session and old_session["mode"] in self.modes:
            self.mode = old_session["mode"]
        else:
            self.mode = "csharp" if "csharp" in self.modes else list(self.modes.keys())[0]
    ...
    def toggle_theme(self):
        """
        Switches between themes (night or high-contrast). Light mode has been removed.
        """
        if self.theme == "night":
            self.theme = "contrast"
        else:
    ...
    def check_code(self):
        """
        Check C# code for build issues.
        """
        current_mode = self.modes.get(self.mode)
        if current_mode and hasattr(current_mode, "check"):
            current_mode.check()
    ...
    def tidy_code(self):
        """
        Prettify C# code with dotnet format.
        """
        current_mode = self.modes.get(self.mode)
        if current_mode and hasattr(current_mode, "tidy"):
            current_mode.tidy()

    def has_python_extension(self, filename):
        """
        Check whether the given filename matches recognized C# / project extensions.
        """
